(function ($) {
    ...
    $(document).on('input', 'input[name="article_no"]', function () {
        let $this = $(this);
        getItemDetails($this, 'article_no');
    });

    $(document).on('keydown', '#article_no', function (event) {
        if (event.key === 'Enter') {
...
function getAjaxResults() {
    ...
    if(exp){
      ...
      const url = 'ajax/sale-creation-results?function=get_results&' + formData;
    ...
    $.ajax({
        type: 'POST',
        url: 'ajax/sale-creation-results?function=get_results',
        data: formData,
...
function getItemDetails($this, type = '') {
    ...
    if (input.length > 0) {
        $.ajax({
            url: 'get_ajaxdata?function=get_purchase_details',
            method: 'POST',
            data: { term: input, type: type },
            success: function (res) {
                let result = JSON.parse(res);
...
function show_item_detail() {
    var barcode = $("#barcode_number").val().trim();
    ...
    $.ajax({
        type: 'POST',
        dataType: "json",
        url: 'get_ajaxdata?function=get_purchage_info',
        data: { barcode: barcode, gst_type: gst_type, under_gst: under_gst, trcount: trcount, act: "get_purchage_info" },
        success: function (result) {
            if (result.status === true) {
                ...
                // safe attribute-based lookup instead of ID selector
                var existingRow = $('#dataTable tr[data-barcode="' + barcode + '"]');
...
function applyAdjustmentAmount() {
    var val = $('#adjustment_amount').val();
    var adjust = parseFloat(val) || 0;
    if (adjust < 0) {
        adjust = 0;
        $('#adjustment_amount').val(0);
    }

    // keep the hidden/main discount_total_amount field in sync
    // so it still submits correctly and total_hidden_posted() picks it up
    $('#discount_total_amount').val(adjust.toFixed(2));

    // keep percent_total_discount in sync too, same logic as discountTotalAmount()
    var totalPrice = 0;
    $(".amount_array").each(function () {
        totalPrice += Number($(this).val()) || 0;
    });
    totalPrice += extra_amount();

    if (totalPrice <= 0) {
        $('#percent_total_discount').val(0);
    } else {
        var discountPercentage = (adjust / totalPrice) * 100;
        $('#percent_total_discount').val(discountPercentage.toFixed(2));
    }
...
function updateBillType() {
    ...
    $("#dataTable tr").each(function () {
        updateTotalPrice($(this), bill_type, 1);
    });
}
...
function change_under_gst() {
    ...
    $("#dataTable tr").each(function () {
        ...
        var gstTitle = (under_gst == 'Local')
            ? "CGST: " + (gstvalue / 2) + "%, SGST: " + (gstvalue / 2) + "%"
            : "IGST: " + gstvalue + "%";
...
console.log(bill_type, net_amount, gst, gst_amount, amount);
...
function total_hidden_posted() {
    ...
    $(".qty_array").each(function () { qArray += Number($(this).val()) || 0; });
    $(".net_array").each(function () { nArray += Number($(this).val()) || 0; });
    $(".gstval").each(function () { gArray += Number($(this).val()) || 0; });
    $(".taxable_amount").each(function () { taxArray += Number($(this).val()) || 0; });
    $(".amount_array").each(function () { amtArray += Number($(this).val()) || 0; });
    ...
    // keep sidebar Adjustment amount field visually in sync
    $('#adjustment_amount').val(adjust.toFixed(2));
    ...
    $('#sub_total_amount').text('₹' + amtArray.toFixed(2));
...
function updatePopupBillAmount() {
    ...
    var total = parseFloat($('#total_amount2').text().replace('₹', '').trim()) || 0;
...
function showItemByArticle(sku) {
    ...
    $.ajax({
        type: 'POST',
        dataType: 'json',
        url: 'get_ajaxdata?function=get_purchage_info_by_sku',
...
function validateAndSubmitPopup() {
    ...
    if (!payMode || payMode.value === '') {
        ...
        activeFields.forEach(function(id){
            ...
            if (el && el.value.trim() !== '') {
                anyFilled = true;
                totalPayment += parseFloat(el.value) || 0;
            }
        });
        if (!anyFilled) {
        ...
        // Check payment amount does not exceed bill amount
        if (anyFilled) {
            var billAmountText = document.getElementById('popup_bill_amount').textContent;
            var billAmount = parseFloat(billAmountText.replace('₹', '').trim()) || 0;
            if (totalPayment > billAmount) {
                errors.push('Total payment amount (₹' + totalPayment.toFixed(2) + ') cannot exceed bill amount (₹' + billAmount.toFixed(2) + ')');
                activeFields.forEach(function(id){
                    var el = document.getElementById(id);
                    if (el && !el.disabled) el.style.borderColor = '#e53935';
                });
    ...
    // All validation passed - prepare and submit form
    errorBanner.style.display = 'none';
    
    // Copy popup payment data to hidden form fields (with null checks)
    var acCodeField = document.querySelector('#modernPopup-save input[name="ac_code"]');
    if (acCodeField && document.getElementById('popup_ac_code_hidden')) {
        document.getElementById('popup_ac_code_hidden').value = acCodeField.value || '';
    }
    
    var accountNameField = document.querySelector('#modernPopup-save input[name="account_name"]');
    if (accountNameField && document.getElementById('popup_account_name_hidden')) {
        document.getElementById('popup_account_name_hidden').value = accountNameField.value || '';
    }
    
    var mobileNumberField = document.querySelector('#modernPopup-save input[name="mobile_number"]');
    if (mobileNumberField && document.getElementById('popup_mobile_number_hidden')) {
        document.getElementById('popup_mobile_number_hidden').value = mobileNumberField.value || '';
    }
    
    var payModeField = document.getElementById('payment_mode_select');
    if (payModeField && document.getElementById('popup_pay_mode_hidden')) {
        document.getElementById('popup_pay_mode_hidden').value = payModeField.value || '';
    }
    
    var payCashField = document.getElementById('pay_cash');
    if (payCashField && document.getElementById('popup_pay_cash_hidden')) {
        document.getElementById('popup_pay_cash_hidden').value = payCashField.value || '';
    }
    
    var payUpiField = document.getElementById('pay_upi');
    if (payUpiField && document.getElementById('popup_pay_upi_hidden')) {
        document.getElementById('popup_pay_upi_hidden').value = payUpiField.value || '';
    }
    
    var payCardField = document.getElementById('pay_card');
    if (payCardField && document.getElementById('popup_pay_card_hidden')) {
        document.getElementById('popup_pay_card_hidden').value = payCardField.value || '';
    }
    
    var payCreditField = document.getElementById('pay_credit');
    if (payCreditField && document.getElementById('popup_pay_credit_hidden')) {
        document.getElementById('popup_pay_credit_hidden').value = payCreditField.value || '';
    }
    
    // Copy Additional Data sidebar fields to hidden form fields (with null checks)
    var sidebarPurchaseDate = document.getElementById('purchage_date');
    if (sidebarPurchaseDate && document.getElementById('sidebar_purchage_date_hidden')) {
        document.getElementById('sidebar_purchage_date_hidden').value = sidebarPurchaseDate.value || '';
    }
    
    var sidebarBillNumber = document.getElementById('bill_number');
    if (sidebarBillNumber && document.getElementById('sidebar_bill_number_hidden')) {
        document.getElementById('sidebar_bill_number_hidden').value = sidebarBillNumber.value || '';
    }
    
    var sidebarSCode = document.getElementById('s_code');
    if (sidebarSCode && document.getElementById('sidebar_s_code_hidden')) {
        document.getElementById('sidebar_s_code_hidden').value = sidebarSCode.value || '';
    }
    
    var sidebarStoreName = document.getElementById('store_name');
    if (sidebarStoreName && document.getElementById('sidebar_store_name_hidden')) {
        document.getElementById('sidebar_store_name_hidden').value = sidebarStoreName.value || '';
    }
    
    var sidebarLocation = document.getElementById('location');
    if (sidebarLocation && document.getElementById('sidebar_location_hidden')) {
        document.getElementById('sidebar_location_hidden').value = sidebarLocation.value || '';
    }
    
    var sidebarGstType = document.querySelector('[name="gst_type"]');
    if (sidebarGstType && document.getElementById('sidebar_gst_type_hidden')) {
        document.getElementById('sidebar_gst_type_hidden').value = sidebarGstType.value || '';
    }
    
    var sidebarUnderGst = document.querySelector('[name="under_gst"]');
    if (sidebarUnderGst && document.getElementById('sidebar_under_gst_hidden')) {
        document.getElementById('sidebar_under_gst_hidden').value = sidebarUnderGst.value || '';
    }
    
    var sidebarBillType = document.querySelector('[name="bill_type"]');
    if (sidebarBillType && document.getElementById('sidebar_bill_type_hidden')) {
        document.getElementById('sidebar_bill_type_hidden').value = sidebarBillType.value || '';
    }
    
    var sidebarPackingAmount = document.getElementById('packing_amount');
    if (sidebarPackingAmount && document.getElementById('sidebar_packing_amount_hidden')) {
        document.getElementById('sidebar_packing_amount_hidden').value = sidebarPackingAmount.value || '';
    }
    
    var sidebarDeliveryAmount = document.getElementById('delivery_amount');
    if (sidebarDeliveryAmount && document.getElementById('sidebar_delivery_amount_hidden')) {
        document.getElementById('sidebar_delivery_amount_hidden').value = sidebarDeliveryAmount.value || '';
    }
    
    var sidebarOtherAmount = document.getElementById('other_amount');
    if (sidebarOtherAmount && document.getElementById('sidebar_other_amount_hidden')) {
        document.getElementById('sidebar_other_amount_hidden').value = sidebarOtherAmount.value || '';
    }
    
    var sidebarLabourAmount = document.getElementById('labour_amount');
    if (sidebarLabourAmount && document.getElementById('sidebar_labour_amount_hidden')) {
        document.getElementById('sidebar_labour_amount_hidden').value = sidebarLabourAmount.value || '';
    }
    
    var sidebarDiscountTotal = document.getElementById('discount_total_amount');
    if (sidebarDiscountTotal && document.getElementById('sidebar_discount_total_amount_hidden')) {
        document.getElementById('sidebar_discount_total_amount_hidden').value = sidebarDiscountTotal.value || '';
    }
    
    var sidebarRemarks = document.getElementById('remarks');
    if (sidebarRemarks && document.getElementById('sidebar_remarks_hidden')) {
        document.getElementById('sidebar_remarks_hidden').value = sidebarRemarks.value || '';
    }
    
    // Ensure total_hidden_posted is called to populate all hidden fields with latest data
    total_hidden_posted();
    
    // Submit the form with all data - use native HTMLFormElement.prototype.submit to bypass naming conflicts
    console.log('Submitting form with all data...');
    try {
        var form = document.querySelector('form[name="purchage_form"]');
        if (form) {
...
function openCategoryPopup(btn) {
    document.getElementById('grp_error_msg').style.display = 'none';

    ['grp_group','grp_sub_category','grp_brand','grp_color','grp_size','grp_style'].forEach(function(id){
        var el = document.getElementById(id);
        el.value = '';
        el.style.borderColor = '';
    });
    ...
    if (_activeGroupRow) {
        var gr = _activeGroupRow.find('input[name="group_name[]"]').val();
        var sc = _activeGroupRow.find('input[name="subgroup[]"]').val();
        var br = _activeGroupRow.find('input[name="brand[]"]').val();
        var co = _activeGroupRow.find('input[name="color[]"]').val();
        var sz = _activeGroupRow.find('input[name="size[]"]').val();
        var st = _activeGroupRow.find('input[name="style[]"]').val();
        if (gr) document.getElementById('grp_group').value = gr;
        if (sc) document.getElementById('grp_sub_category').value = sc;
        if (br) document.getElementById('grp_brand').value = br;
        if (co) document.getElementById('grp_color').value = co;
        if (sz) document.getElementById('grp_size').value = sz;
        if (st) document.getElementById('grp_style').value = st;
    }
...
function saveGroupDetails() {
    var group  = document.getElementById('grp_group').value.trim();
    var subCat = document.getElementById('grp_sub_category').value.trim();
    var brand  = document.getElementById('grp_brand').value.trim();
    var color  = document.getElementById('grp_color').value.trim();
    var size   = document.getElementById('grp_size').value.trim();
    ...
    _activeGroupRow.find('input[name="group_name[]"]').val(group);
    _activeGroupRow.find('input[name="subgroup[]"]').val(subCat);
    _activeGroupRow.find('input[name="brand[]"]').val(brand);
    _activeGroupRow.find('input[name="color[]"]').val(color);
    _activeGroupRow.find('input[name="size[]"]').val(size);
    _activeGroupRow.find('input[name="style[]"]').val(style);

    var btn = _activeGroupRow.find('.plus-btn').first();
    if (btn.length) {
        var summary = subCat + (group ? ' | ' + group : '') + (brand ? ' | ' + brand : '') + (color ? ' | ' + color : '') + (size ? ' | ' + size : '') + (style ? ' | ' + style : '');
        btn.attr('title', 'Add Category Details (' + summary + ')');
        btn.css('box-shadow', '0 0 0 3px #009688');
        setTimeout(function(){ btn.css('box-shadow', ''); }, 1500);
    }
...
function openPopupsp(btn) {
    ...
    if (_activeMRPRow) {
        ...
        var dp  = parseFloat(_activeMRPRow.find('.discount_percent2').val()) || 0;
        var da  = parseFloat(_activeMRPRow.find('.discount_amount2').val()) || 0;
        var sp  = _activeMRPRow.find('.selling_price').val();

        document.getElementById('mrp_current').value = mrp.toFixed(2);
        if (dp > 0) document.getElementById('mrp_discount_pct').value = dp;
        if (da > 0) document.getElementById('mrp_discount_amt').value = da;
        // only show SP if the user already set it before; otherwise leave blank
        document.getElementById('mrp_selling_price').value = sp ? sp : '';
    }
    document.getElementById('modernPopupsp').classList.add('active');
    document.getElementById('mrp_discount_pct').focus();
}

function closePopupsp() {
    document.getElementById('modernPopupsp').classList.remove('active');
}
...
function saveMRPDetails() {
    ...
    if (!_activeMRPRow || _activeMRPRow.length === 0) { closePopupsp(); return; }
    ...
    _activeMRPRow.find('.selling_price').css('background', '#e6f7f5');
    setTimeout(function(){ _activeMRPRow.find('.selling_price').css('background', ''); }, 1500);

    // Call updateTotalPrice with current row and bill type
    var bill_type = $('#gst_type').find(':selected').val();
    updateTotalPrice(_activeMRPRow, bill_type);
