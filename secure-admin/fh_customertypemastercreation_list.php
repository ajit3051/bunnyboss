...
{
   $id=$_GET['delete_id'];
   $sqld="DELETE FROM tbl_customertype_master WHERE id='$id'";
   $res=mysqli_query($conn,$sqld);
   if($res)
   {
    header('refresh:.5; url=fh_customertypemastercreation_list.php');
   }else{
    echo "<script>alert('Customertype Master Record Failed');</script>";
   }
}
?>
...
<head>
      ...
      <title>Customer Type Master List || TEJASERP</title>
