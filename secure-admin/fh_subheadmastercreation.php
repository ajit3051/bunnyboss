...
<?php
    $msg='';
     ...
     {
      $subhead_code = mysqli_real_escape_string($conn, $_POST['subhead_code']);
      $head_name    = mysqli_real_escape_string($conn, $_POST['head_name']);
      $subhead_name = mysqli_real_escape_string($conn, $_POST['subhead_name']);
      ...
      // $picture=implode(' ',$data);
          ...
          $dup=mysqli_query($conn,"select * from tbl_subhead_master where subhead_name='$subhead_name'");
            if(mysqli_num_rows($dup)>0)
            {
                $msg='<div class="alert alert-danger alert-dismissible fade1 show">
                    <button type="button" class="close" data-dismiss="alert">&times;</button>
                    <strong>SubHead Name Already exist!</strong>
                    </div>';
            }
            else{
                $query="Insert into tbl_subhead_master(subhead_code,head_name,subhead_name,status)values('$subhead_code','$head_name','$subhead_name','$status')";
        ...
        if($run)
   {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        showPopup(
            'Success!',
            'SubHead Name Saved Successfully 🎉',
    ...
    ";
                    ...
                    $msg='<div class="alert alert-danger alert-dismissible fade1 show">
                    <button type="button" class="close" data-dismiss="alert">&times;</button>
                    <strong>SubHead Name Creations Failed!</strong>
                    </div>';
                }
            } 
        }
...
<head>
      ...
      <title>SubHead Master Creation || Fashion Hub</title>
                        ...
                        <div class="panel-body form-scroll">
                           ...
                           <form method="post">
                              ...
                              <div class="row">
                                 <div class="col-sm-4">
                                    <div class="form-group">
                                                        <label>SubHead Code</label><label style="color:red;">*</label>
                                                        <input type="password" name="subhead_code" class="form-control" id="subhead_code" readonly value="<?php @$no = mysqli_query($conn, "SELECT id FROM  tbl_subhead_master ORDER BY id DESC");
                                                        $po_no = ($no) ? mysqli_fetch_assoc($no) : null;
                                                        $next_id = ($po_no && isset($po_no['id'])) ? ($po_no['id'] + 1) : 1;
                                                        echo 'SHC-000' . $next_id;
                                                    ?>">
                                                </div>
                                             </div>
                                             <div class="col-lg-4">
                                 ...
                                 <?php } ?> 
                                             ...
                                             <div class="col-sm-4">
                                                <div class="form-group">
                                                   ...
                                                   <input type="text" class="form-control" name="subhead_name" id="subhead_name" placeholder="Enter Your SubHead Name" >
                                                </div>
                                             </div>
