...
<?php
   ...
   {
      $id=$_GET['delete_id'];
      $sqld="DELETE FROM tbl_storeprofile_master WHERE id='$id'";
      $res=mysqli_query($conn,$sqld);
      if($res)
      {
       header('refresh:.5; url=fh_storeprofilecreation_list.php');
      }else{
       echo "<script>alert('Store Profile Record Failed');</script>";
      }
   }
   ?>
<!DOCTYPE html>
<html lang="en">
   ...
   <head>
      ...
      <title>Store Profile Master List || TEJASERP</title>
