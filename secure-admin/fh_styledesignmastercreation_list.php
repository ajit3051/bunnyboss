...
<?php
   ...
   {
      $id=$_GET['delete_id'];
      $sqld="DELETE FROM tbl_styledesign_master WHERE id='$id'";
      $res=mysqli_query($conn,$sqld);
      if($res)
      {
       header('refresh:.5; url=fh_styledesignmastercreation_list.php');
      }else{
       echo "<script>alert('Style || Design Master Record Failed');</script>";
      }
   }
   ?>
<!DOCTYPE html>
<html lang="en">
   ...
   <head>
      ...
      <title>Style || Design Master List || TEJASERP</title>
