...
{
  $id=$_GET['update_id'];
  $run=mysqli_query($conn,"select * from tbl_head_master where id='$id'");
...
if(isset($_POST['update']))
   {
    ...
    $head_name = str_replace("'", "\'", $head_name);
    $head_name = str_replace('"', '\"', $head_name);

        
    $query="UPDATE tbl_head_master SET head_code='$head_code', head_name='$head_name',status='$status' WHERE id='$id'";
    if (mysqli_query($conn,$query)){
        $msg='<div class="alert alert-success alert-dismissible fade1 show">
                    <button type="button" class="close" data-dismiss="alert">&times;</button>
                    <strong>Head Name Update..!</strong>
                </div>';
                header('refresh:.5; url=fh_headmastercreation_list.php');
    }
    else{
...
<head>
      ...
      <title>Head Master Update || Fashion Hub</title>
                              ...
                              <div class="row">
                                 <div class="col-sm-4">
                                    <div class="form-group">
                                            ...
                                            <input type="password" name="head_code" class="form-control" readonly value="<?php echo $user['head_code'];?>" />
                                            </div>
                                         </div>
                                         <div class="col-sm-8">
                                          <div class="form-group">
                                                <label>Head Name</label>
                                                <input type="text" class="form-control" name="head_name" id="head_name"  value="<?php echo $user['head_name'];?>" placeholder="Enter Your Head Name" required>
                                            </div>
                                         </div>
