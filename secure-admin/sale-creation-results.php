...
if ($exportType) {
   $columns = 'purchage_date, bill_number, account_name, pay_mode, gst_type, gst_amount, SUM(amount) as amount, status';
} else {
   $columns = 'id, purchage_date, bill_number, account_name, pay_mode, gst_type, gst_amount, SUM(amount) as amount, status';
}
...
if ($exportType == 'excel') {
   exportResult('sale-item', $stmt);
   exit();
}
if($exportType == 'pdf'){
   exportResultPdf('sale-item', $stmt); 
   exit();
}
...
if ($totalRecordsWithLimit > 0) {
   ...
   for ($i = 1; $i <= $totalRecordsWithLimit; $i++) {
      ...
      $html .= '<tr>
         ...
         <td>' . getPayModeByCode($row['pay_mode']) . '</td>
         <td>' . $row['gst_type'] . '</td>
